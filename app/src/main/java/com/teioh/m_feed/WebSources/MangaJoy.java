...
public class MangaJoy {
    ...
    */
     ...
     */
    public static Observable<String> getMangaDescriptionObservable(final List<Manga> url) {
        return pullMangaDescriptionFromWebsite(url)
                ...
                try {
                    for (Manga m : url) {
                        if (m.getmGenre() == null) {
                            ...
                            subscriber.onNext(parseHtmlToDescription(unparsedHtml, m.getMangaURL()));
                        }
                    }
                    subscriber.onCompleted();
                } catch (Throwable e) {
    ...
    private static String parseHtmlToDescription(final String unparsedHtml, String url) {
        ...
        values.put("mGenre", genres);
        values.put("mStatus", status);
        values.put("mSource", source);

        int x = cupboard().withDatabase(MangaFeedDbHelper.getInstance().getWritableDatabase()).update(Manga.class, values, "mMangaUrl = ?", url);

        Log.e("something is happeing", Integer.toString(x));
        return null;
    }
